...
def do_lambda_form(expressions, env):
    ...
    # body attribute of a Lamda Procedure instance is a Scheme list of body expressions
    # formals attribute of a LambdaProcedure is a prperly nested Pair expression
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    eval_item = scheme_eval(expressions.first, env)
    if is_scheme_true(eval_item):
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        arg = bindings.first.first
        values = scheme_eval(bindings.first.rest.first, env)
        names = Pair(arg, names)
        vals = Pair(values, vals)
...
def do_mu_form(expressions, env):
    ...
    validate_formals(formals)
    # BEGIN PROBLEM 11
    # mu form evaluates to a mu procedure
